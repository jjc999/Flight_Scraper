...
class seleniumFS:
  def __init__(self):
    ...
    self.xPaths = {'departure_airport': "//*[@id='i14']/div[6]/div[2]/div[2]/div[1]/div/input",
                   ...
                   'departure_date': "//*[@id='ow62']/div[1]/div/div/div[1]/div/div[1]/div/input"}
  ...
  def input_user_data(self, user):
    ...
    # -----------------------------
    # I figured out that the departure airport box is the default active element, so I can just click on that to be able to input text.
    # Once I click on it, I am able to get to the text element that I can actually use Selenium to interact with (what the xpath now 
    # references).  Once I get that, I can send the departure airport text to the box.  I then click ENTER to return to the search 
    # page and then hit TAB to move over to the arrival airport box.  My strategy now is to try finding the xpaths for each element 
    # following this approach and to google around and see if other people have done this and can just give me the xpaths.
    # -----------------------------
    ...
    arrival_airport_box.send_keys(Keys.TAB)
    # departure_date_box = self.browser.find_element("xpath", self.xPaths['departure_date'])
    departure_date_box = self.browser.switch_to.active_element
